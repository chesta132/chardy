...
import { ReplyEnvelope } from "../reply/types";
import { ResponseError, ResponseSuccess } from "./serverResponse";
...
export type ApiConfig<B = any, Q = any, P = any, H = any> = Omit<AxiosRequestConfig, "data"> &
...
export class ApiClient {
  ...
  constructor(baseURL: string) {
    ...
    this.api.interceptors.response.use(
      (res) => {
        if (isDevEnv()) console.log(`Server request success.\n${res.config.method} ${res.config.url}\n`, res.data);
        return res;
      },
      (error) => {
        if (isDevEnv())
          console.error(`Server request failed.\n${error.config?.method} ${error.config?.url}\n`, {
            ...pick(error, ["code", "message", "status"]),
  ...
  private async request<T>({ query, param, url, header, method, data, ...config }: ApiConfig): Promise<ResponseSuccess<T>> {
    try {
      ...
      })) as AxiosResponse<ReplyEnvelope<T, true>>;
      return new ResponseSuccess(response, validator.response);
    } catch (error) {
